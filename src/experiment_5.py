...
def kappa(parameters, x, y):
    ...
    x_points = jnp.linspace(-3, 3, N)
    y_points = jnp.linspace(-3, 3, N)
    values = params.reshape((N, N))
...
def f_full(parameters, x, y, L):
    return jnp.sin(2 * jnp.pi * x / L) * jnp.sin( 2 * jnp.pi * y / L)
# -----------------------------------------------------------------------------  
...
if __name__ == "__main__":
    ...
    if input("Run the experiment? (y/n): ") == "y":
        ...
        # Save the results.
        jnp.save("src/files/experiment_1/hybrid_loss.npy", loss_history_hyb)
        jnp.save("src/files/experiment_1/hybrid_params.npy", param_history_hyb)
        jnp.save("src/files/experiment_1/fem_loss.npy", loss_history_fem)
        ...
        loss_history_pinn = jnp.load("src/files/experiment_1/pinn_loss.npy")
        param_history_hyb = jnp.load("src/files/experiment_1/hybrid_params.npy")
        param_history_fem = jnp.load("src/files/experiment_1/fem_params.npy")
        param_history_pinn = jnp.load("src/files/experiment_1/pinn_params.npy")

...
def kappa(parameters, x, y): 
    amplitude1, cx1, cy1 = parameters[0:3]
    amplitude2, cx2, cy2 = parameters[6:9]
    return amplitude1 * jnp.exp(-(((x - cx1)**2 + (y - cy1)**2))) + amplitude2 * jnp.exp(-(((x - cx2)**2 + (y - cy2)**2))) + 1
...
def f_full(parameters, x, y, L):
    A, ax, ay, B, bx, by, C, cx, cy = parameters
...
def train_hybrid(epochs):
    ...
    amplitudes = jax.random.uniform(rng1, shape=(3,), minval=1, maxval=3)
    centers_x = jax.random.uniform(rng2, shape=(3,), minval=-1, maxval=1)
    centers_y = jax.random.uniform(rng3, shape=(3,), minval=-1, maxval=1)
    init_params = nnx.Param(jnp.array([
        amplitudes[0], centers_x[0], centers_y[0],
        amplitudes[1], centers_x[1], centers_y[1],
        amplitudes[2], centers_x[2], centers_y[2]
    ]))
...
if __name__ == "__main__":
    ...
    if input("Run the experiment? (y/n): ") == "y":
        ...
        # Save the results.
        jnp.save("src/files/experiment_3/hybrid_loss.npy", loss_history_hyb)
        jnp.save("src/files/experiment_3/hybrid_params.npy", param_history_hyb)
        jnp.save("src/files/experiment_3/fem_loss.npy", loss_history_fem)
        jnp.save("src/files/experiment_3/fem_params.npy", param_history_fem)
        jnp.save("src/files/experiment_3/pinn_loss.npy", loss_history_pinn)
        jnp.save("src/files/experiment_3/pinn_params.npy", param_history_pinn)
    else:
        loss_history_hyb = jnp.load("src/files/experiment_3/hybrid_loss.npy")
        loss_history_fem = jnp.load("src/files/experiment_3/fem_loss.npy")
        loss_history_pinn = jnp.load("src/files/experiment_3/pinn_loss.npy")
        param_history_hyb = jnp.load("src/files/experiment_3/hybrid_params.npy")
        param_history_fem = jnp.load("src/files/experiment_3/fem_params.npy")
        param_history_pinn = jnp.load("src/files/experiment_3/pinn_params.npy")
    ...
    domain=(-3.0, 3.0),
    N=100,
    filename="experiment_3/experiment_3"
    )

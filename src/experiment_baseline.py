...
n_train = 10
x_train = jnp.linspace(-3.0, 3.0, n_train, endpoint=True)
y_train = jnp.linspace(-3.0, 3.0, n_train, endpoint=True)
...
def pinn_training():
    """
    Trains a PINN on the experiment_baseline setup.
    """
    model_syn = ResNetSynthetic(
    num_blocks=3,
    features=100,
...
if __name__ == "__main__":
    if input("Reproduce data? (y/n): ") == "y":
        ...
        # Save all in files
        jnp.save("src/files/experiment_baseline/helmholtz_hybrid_loss.npy", hyb_phys_loss_hist)
        jnp.save("src/files/experiment_baseline/helmholtz_hybrid_metrics.npy", hyb_metrics_hist)
        jnp.save("src/files/experiment_baseline/helmholtz_hybrid_state_hist.npy", hyb_state_hist)

        jnp.save("src/files/experiment_baseline/helmholtz_phys_loss.npy", phys_loss_hist)
        jnp.save("src/files/experiment_baseline/helmholtz_phys_state_hist.npy", phy_state_hist)

        jnp.save("src/files/experiment_baseline/helmholtz_pinn_loss.npy", pinn_loss_hist)
        jnp.save("src/files/experiment_baseline/helmholtz_pinn_state_hist.npy", pinn_state_hist)
        

    else:
        hyb_phys_loss_hist = jnp.load("src/files/experiment_baseline/helmholtz_hybrid_loss.npy", allow_pickle=True)
        hyb_metrics_hist = jnp.load("src/files/experiment_baseline/helmholtz_hybrid_metrics.npy", allow_pickle=True)
        hyb_state_hist = jnp.load("src/files/experiment_baseline/helmholtz_hybrid_state_hist.npy", allow_pickle=True)

        phys_loss_hist = jnp.load("src/files/experiment_baseline/helmholtz_phys_loss.npy", allow_pickle=True)
        phy_state_hist = jnp.load("src/files/experiment_baseline/helmholtz_phys_state_hist.npy", allow_pickle=True)

        pinn_loss_hist = jnp.load("src/files/experiment_baseline/helmholtz_pinn_loss.npy", allow_pickle=True)
        pinn_state_hist = jnp.load("src/files/experiment_baseline/helmholtz_pinn_state_hist.npy", allow_pickle=True)

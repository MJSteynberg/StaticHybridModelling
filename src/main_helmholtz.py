...
if __name__ == "__main__":
    ...
    # Save all in files
    jnp.save("src/files/helmholtz_hybrid_loss.npy", hyb_phys_loss_hist)
    jnp.save("src/files/helmholtz_hybrid_metrics.npy", hyb_metrics_hist)
    jnp.save("src/files/helmholtz_hybrid_state_hist.npy", hyb_state_hist)

    jnp.save("src/files/helmholtz_phys_loss.npy", phys_loss_hist)
    jnp.save("src/files/helmholtz_phys_state_hist.npy", phy_state_hist)

    plot(state_phys, state_hyb, phys_loss_hist, hyb_phys_loss_hist, true_params, pts_train,
         gaussian_kappa, gaussian_eta, "helmholtz.png")
    
    animate_both_evolution(
        phy_state_history=phy_state_hist,      # your list of physics states
        hyb_state_history=hyb_state_hist,      # your list of hybrid states
        ...
        domain=(-3.0, 3.0),
        N=100,
        filename="helmholtz_training.gif"
    )
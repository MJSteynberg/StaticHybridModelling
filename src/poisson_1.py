...
def f(parameters, x, y):
    A, kx, ky = parameters
    return A * jnp.sin(kx*x) * jnp.sin(ky*y)
...
def train_hybrid(epochs):
    ...
    init_params = nnx.Param(jax.random.uniform(jax.random.PRNGKey(0), shape=(3,)))
    ...
    param_history = np.zeros((epochs, 3))
...
if __name__ == "__main__":
    ...
    if input("Train the hybrid model? (y/n): ") == "y":
        ...
        checkpoint_path = os.path.abspath("src/files/poisson_1/checkpoint")
        ckpt_dir = ocp.test_utils.erase_and_create_empty(checkpoint_path)
        checkpointer.save(ckpt_dir / 'state', state)
    else:
        loss_history_hyb = jnp.load("src/files/poisson_1/loss_history_hyb.npy")
        param_history_hyb = jnp.load("src/files/poisson_1/param_history_hyb.npy")
    ...
    plt.savefig("src/results/poisson_1/prediction_comparison.png")
